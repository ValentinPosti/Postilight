  ...
  */
  ...
  if (g_index_row < 16)
  {
    for (k = 0; k < 8; k++)
    {
      ...
      g_buffer_txt[((g_index_row + (k * 16)) * 3) + 0 + (16 * 3 * 4)] = R; //le 64 c'est pour commencer l'affichage en sautant 4 lignes et ainsi centrer le texte en vertical
      g_buffer_txt[((g_index_row + (k * 16)) * 3) + 1 + (16 * 3 * 4)] = G;
      g_buffer_txt[((g_index_row + (k * 16)) * 3) + 2 + (16 * 3 * 4)] = B;
    }
  }
  g_index_row += 1;
}
...
{
  sendBitx8(row, r, g, b); // WS2812 takes colors in GRB order
}
...
{
  unsigned int l = NB_PIXELS / (FONT_WIDTH + INTERCHAR_SPACE);

  sendChar(*s, skip, r, g, b); // First char is special case because it can be stepped for smooth scrolling
